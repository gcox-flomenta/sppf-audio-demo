...
if [ "$FLAC_COUNT" -gt 50 ]; then
    ...
    # Try GitHub Releases cache first (much faster than OpenSLR)
    if command -v gh &>/dev/null; then
        log "Checking GitHub 'datasets' release for cached LibriSpeech..."
        gh release download "datasets" --repo "$GH_REPO" --pattern "librispeech_*.tar.gz" --dir /workspace 2>/dev/null
        PARTS=$(ls /workspace/librispeech_*.tar.gz 2>/dev/null | sort)
        if [ -n "$PARTS" ]; then
            log "Downloaded $(echo "$PARTS" | wc -l) parts from GitHub — reassembling..."
    ...
    if [ "$RESTORED" = "false" ]; then
        # Download from OpenSLR via aria2c
